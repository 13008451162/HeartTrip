 ...
 *
 * @author: lukecc0
 * @data:2024/3/14 下午3:55
 * @about: TODO  按字母排序的市级城市列表
 */

public class CityOfCollect {

    private static String TAG = "热门城市";

    public static final int TYPE_DATA = 0;
    public static final int TYPE_HEADER = 1;
    public static final int TYPE_RECOMMEND_HEADER = 2;
    public static final int TYPE_RECOMMEND_DATA = 3;

    private static final String[] recommendCity = {
            "北京", "上海", "重庆", "武汉", "拉萨", "西安", "深圳", "成都"
    };

    private static final String[] municipalCity = {
     ...
     * 获取一个存储城市的列表
     *
     * @return {@link ArrayList}<{@link CityData}>
     */
    public static ArrayList<CityData> getCityList() {
        ArrayList<CityData> mCityDataList = new ArrayList<>();

        mCityDataList.add(new CityData(TYPE_RECOMMEND_HEADER, TAG));

        for (String city : municipalCity) {
            if (isLetter(city)) {
                mCityDataList.add(new CityData(TYPE_HEADER, city));
            } else {
                mCityDataList.add(new CityData(TYPE_DATA, city));
            }
        }
     ...
     * @return {@link ArrayList}<{@link CityData}>
     */
    public static ArrayList<CityData> getRecommendCity() {
        ArrayList<CityData> arrayList = new ArrayList<>();
        for (String s : recommendCity) {
            arrayList.add(new CityData(TYPE_RECOMMEND_DATA, s));
        }
        return arrayList;
    }

    public static int getNumberOfPopularCities() {
        return recommendCity.length;
    }
